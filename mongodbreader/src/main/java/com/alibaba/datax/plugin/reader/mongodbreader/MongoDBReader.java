...
import com.alibaba.datax.plugin.reader.mongodbreader.util.DocumentToBigString;
...
public class MongoDBReader extends Reader {
	...
	public static class Task extends Reader.Task {
		...
		private String query_time = null;
		...
		public void startRead(RecordSender recordSender) {
			...
			if (!Strings.isNullOrEmpty(query)) {
				...
				try {
					if (queryFilter.containsKey(query_time)) {
						Document timeFilter = (Document) queryFilter.get(query_time);
						SimpleDateFormat formatter = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss.SSS");
						String tStart = timeFilter.getString("$gt");
						Date t_start = formatter.parse(tStart);
						String tEnd = timeFilter.getString("$lte");
						Date t_end = formatter.parse(tEnd);
						queryFilter.put(query_time, new Document("$gt", t_start).append("$lte", t_end));
					}
					if (queryFilter.containsKey("_id")) {
						Document idFilter = (Document) queryFilter.get("_id");
						String minId = null;
						String maxId = null;
						String gtOperator = null;
						String ltOperator = null;
						Document queryIdOption = new Document();
						if (idFilter.containsKey("$gt")) {
							gtOperator = "$gt";
							minId = idFilter.getString("$gt");
							queryIdOption.append(gtOperator, new ObjectId(minId));
						}
						if (idFilter.containsKey("$gte")) {
							gtOperator = "$gte";
							minId = idFilter.getString("$gte");
							queryIdOption.append(gtOperator, new ObjectId(minId));
						}
						if (idFilter.containsKey("$lt")) {
							ltOperator = "$lt";
							maxId = idFilter.getString("$lt");
							queryIdOption.append(ltOperator, new ObjectId(maxId));
						}
						if (idFilter.containsKey("$lte")) {
							ltOperator = "$lte";
							maxId = idFilter.getString("$lte");
							queryIdOption.append(ltOperator, new ObjectId(maxId));
						}
						queryFilter.put("_id", queryIdOption);
					}
				} catch (Exception e) {
			...
			if (!Strings.isNullOrEmpty(query_params)) {
				...
				BasicDBObject projection = (BasicDBObject) com.mongodb.util.JSON
						.parse(jsonObject.getString("projection"));
				if (null != sort) {
					documents.sort(sort);
				}
			...
			if (syncType.equals("flatten")) {
					...
					for (Object column : midMongodbColumnMeta) {
						...
						if (type.equals("ArrayObjectLevelOne")) {
							...
							if(nestedObject==null){
								...
								JSONObject originJson = (JSONObject) preJson.clone();
								RecursionDocument.getDoubleListJson(name, nestedObject, subMidList, preJson, originJson,
										false);
							}														
						}
						midList.addAll(subMidList);
					}
					if (midList.size() >= threshold) {
				...
				/*
				 * //重复代码 for (JSONObject _item : midList) { JSONObject item =
				 * dbCursor.next(); Document item =
				 * Document.parse(_item.toJSONString()); }
				 */
			}
		...
		public void init() {
			...
			this.database = readerSliceConfig.getString(KeyConstant.MONGO_DB_NAME,
					readerSliceConfig.getString(KeyConstant.MONGO_DATABASE));
			...
			this.query_time = readerSliceConfig.getString("query_time");
		}
		...
		private void resultSendToWriter(RecordSender recordSender, JSONArray newMongodbColumnMeta, Document item) {
			...
			while (columnItera.hasNext()) {
				...
				if (tempCol == null) {
					...
					String json = ((Document) tempCol).toJson(new JsonWriterSettings(JsonMode.SHELL))
							.replace("ISODate(", "").replace(")", "");
					record.addColumn(new StringColumn(json));
				} else {
					if (KeyConstant.isArrayType(column.getString(KeyConstant.COLUMN_TYPE))) {
						...
						if (Strings.isNullOrEmpty(splitter)) {
							...
							List array = (ArrayList) tempCol;
							if (!array.isEmpty() && array.get(0) instanceof Document) {
								List<JSONObject> jsonObjects = new ArrayList<JSONObject>();
								for (Object doc : array) {
									String json = ((Document) doc).toJson(new JsonWriterSettings(JsonMode.SHELL))
											.replace("ISODate(", "").replace(")", "");
					...
					else if (KeyConstant.isBIG_STRING_TYPEType(column.getString(KeyConstant.COLUMN_TYPE))) {
						record.addColumn(new StringColumn(DocumentToBigString.TransBigString(tempCol)));
					} else {
						// TODO 替换string所带分隔符
						record.addColumn(new StringColumn(tempCol.toString().replaceAll("[\\u0000-\\u001f\b]", "")));
					}
				}
			}
